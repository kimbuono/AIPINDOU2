...
export default function ConfigPanel({
  ...
  return (
    <div className="space-y-7 rounded-2xl border border-neutral-200 bg-white p-6 sm:p-7">
      {/* 色卡品牌 */}
      <fieldset>
        <legend className="mb-3.5 flex items-center gap-2 text-[13px] font-semibold tracking-wider text-neutral-500 uppercase">
          <svg
            className="h-4 w-4"
            viewBox="0 0 24 24"
            fill="none"
        ...
        <div className="grid grid-cols-2 gap-2">
          {BRANDS.map((b) => (
            ...
            >
              ...
              <span
                className={`mt-0.5 block text-[12px] ${brand === b.value ? "text-neutral-400" : "text-neutral-400"}`}
              >
                {b.desc}
              </span>
            </button>
        ...
        <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
          {SIZES.map((s) => (
            <button
              ...
              className={`relative rounded-xl border px-2 py-3 text-center transition-all duration-150 ${
                gridSize === s.value
                  ? "border-neutral-900 bg-neutral-900 text-white shadow-sm"
                  : "border-neutral-200 bg-white text-neutral-600 hover:border-neutral-300 hover:text-neutral-800"
              } `}
            >
              <span className="block text-[14px] leading-tight font-semibold">{s.label}</span>
              <span className="mt-0.5 block text-[11px] opacity-70">共 {s.value * s.value} 颗</span>
            </button>
          ))}
        </div>
      </fieldset>
      ...
      <fieldset>
        ...
        <label className="flex cursor-pointer items-center justify-between py-2">
          <div>
            ...
            <p className="mt-0.5 text-[12px] text-neutral-400">平滑颜色渐变，减少色块感</p>
          </div>
          <button
            type="button"
            onClick={() => onDitherChange(!dither)}
            className={`relative h-6 w-11 rounded-full transition-colors duration-200 ${dither ? "bg-blue-500" : "bg-neutral-300"} `}
          >
            <span
              className={`absolute top-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform duration-200 ${dither ? "translate-x-5.5" : "translate-x-0.5"} `}
              style={{ left: "1px" }}
            />
          </button>
        </label>
